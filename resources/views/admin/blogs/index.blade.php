...
@section('content')
    ...
    <div class="row">
        <div class="col-lg-12 text-right">
            <a href="{{ route('admin.blogs.create') }}" class="btn btn-success"><i class="fa fa-plus-circle"></i> Add blog</a>
        </div>
    </div>
    <br />
    <div class="row">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            <td class="text-center">
                                                {!! $blog->image ? image(config('blog.image_path') . '/tn-' . $blog->image, array('width' => 50), '/img/no_image.png') : '<img src="/img/no_image.png" width="50">' !!}
                                            </td>
                                            <td><a href="{{ route('admin.blogs.show', $blog->id) }}">{{ $blog->title }}</td>
                                            <td>{{ $blog->author->name or '' }}</td>
                                            <td>{{ $blog->updated_at }}</td>
                                            <td>
                                                <a href="{{ route('admin.blogs.edit', $blog->id) }}" class="btn btn-info"><i class="fa fa-edit"></i> Edit</a>
                                            </td>
                                        </tr>
                                        @endforeach
                                    </tbody>
